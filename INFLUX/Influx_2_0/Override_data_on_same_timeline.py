...
def calculate_dates(year, month, week):
    """
        In this function we calculate dates for weekly basis and return start and end dates
    """
    try:
        ...
        end_of_week = min(first_day_of_month + timedelta(days=week * 7 - 1, hours=23, minutes=59, seconds=59),
...
def get_data_from_influx2(source_client, measurement, start_time, end_time, device):
    """
    ...
    try:
        ...
        query = f'from(bucket: "{source_influx2_bucket}") |> range(start: {start_time_str}, stop: {end_time_str}) |> filter(fn: (r) => r["_measurement"] == "{measurement}") |> filter(fn: (r) => r["panel_no"] == "iam-gw-210146") |> filter(fn: (r) => r["device_code"] == "{device}") |> filter(fn: (r) => r["zone"] == "TKW")'
        ...
        for table in data:
            for record in table.records:
                ...
                try:
                    ...
                    if record['_value'] > 1000:
                        influx2_point.field('value', 146.58)
                    else:
...
if __name__ == "__main__":
    ...
    """
    source_influx2_url = 'http://10.129.2.23:8086'
    source_influx2_org = 'iam'
    source_influx2_username = 'root'
    source_influx2_password = 'rootrootroot'
    source_influx2_bucket = 'data'
    source_influx2_token = '9rcqoz4onukDdzgaUJKwkUmZqlzUm3AbwYhR1kVy-2_wWxihEWeAxV4WZVcYJg=='
    ...
    measurements_to_migrate = "power"

    devices_array = ["MOD01", "MOD02", "MOD03", "MOD04", "MOD05", "MOD06", "MOD07", "MOD08", "MOD09", "MOD10", "MOD11"]
