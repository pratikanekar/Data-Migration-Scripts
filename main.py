...
influx2_bucket = 'influx_migration_test'
influx2_token = 'QO2OHg8qo0pxNBH6P9ad7BhZBj9vWYnBuBgqJJ0mb9L_mtUcVFVUKL7a_q7FJ9BWqEJ0v1VtvFqP7miBPKhMEQ=='

# measurements_to_migrate = ['co2', 'current', 'energy', 'event', 'frequency', 'humidity', 'misc', 'netd_info', 'power', 'speed', 'temperature', 'voltage', 'acmode', 'acspeed', 'acstate', 'acswing', 'operation', 'pcmcount']
measurement = 'co2'

def get_data_from_influx1(measurement):
    client = InfluxDBClient(host=influx1_host, port=influx1_port, username=influx1_username, password=influx1_password, database=influx1_database)
    try:
        ...
        result = client.query(f'SELECT * FROM "{measurement}"')  # Use double quotes around the measurement name
...
def write_data_to_influx2(measurement, data):
    client = InfluxDB2Client(url=influx2_url, username= influx2_username, password=influx2_password, token=influx2_token, org=influx2_org)
    try:
...
if __name__ == "__main__":
    data_from_influx1 = get_data_from_influx1(measurement)
