...
# Source InfluxDB 2.0 connection settings
...
source_influx2_bucket = 'icici-ghansoli'
...
def get_data_from_influx2(source_client, measurement, start_time, end_time):
    try:
        # Format the time range in the InfluxDB query language
...
if __name__ == "__main__":
    ...
    # Set up InfluxDB 2.0 source client
    source_client = InfluxDBClient(url=source_influx2_url, token=source_influx2_token, org=source_influx2_org, username=source_influx2_username, password=source_influx2_password, bucket=source_influx2_bucket)

    # Set up InfluxDB 2.0 destination client
    dest_client = InfluxDBClient(url=dest_influx2_url, token=dest_influx2_token, org=dest_influx2_org, username=dest_influx2_username, password=dest_influx2_password, bucket=dest_influx2_bucket)
    ...
    while data_found_flag:
        ...
        if data_from_influx2 is not None:
            ...
            break
        elif data_from_influx2 is None:
